    ...
    private fun setSwipeToDelete() {
        val itemTouchHelperCallback = object: ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                val deletedTask = (binding.todoListRecyclerView.adapter as TaskItemAdapter).removeTask(position)

                // Delete task from ViewModel
                taskViewModel.deleteTask(deletedTask)

                // Show snackbar with undo option
                Snackbar.make(binding.todoListRecyclerView, "Task deleted", Snackbar.LENGTH_LONG)
                    .setAction("Undo") {
                        // Restore task to ViewModel
                        taskViewModel.undoDelete(deletedTask)

                        // Update RecyclerView
                        (binding.todoListRecyclerView.adapter as TaskItemAdapter).restoreTask(deletedTask,position)
                    }.show()
            }
        }
